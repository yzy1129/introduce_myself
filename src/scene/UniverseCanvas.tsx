...
function StarLayer({ depth, count, size, tint, drift }: StarLayerProps) {
  ...
  useFrame((state, delta) => {
    ...
    const targetX = MathUtils.mapLinear(pointerY, 0, window.innerHeight, 0.12, -0.12);
    const targetY = MathUtils.mapLinear(pointerX, 0, window.innerWidth, -0.18, 0.18);
    ...
    points.rotation.x = MathUtils.lerp(points.rotation.x, targetX, delta * 0.8);
    points.rotation.y = MathUtils.lerp(points.rotation.y, targetY, delta * 0.8);
    points.rotation.z += delta * drift * 0.04;
    points.position.x = MathUtils.lerp(
      points.position.x,
      driftX * depthWeight * (performanceTier === "low" ? 0.16 : 0.28),
      delta * 1.2,
    );
    points.position.y = MathUtils.lerp(
      points.position.y,
      Math.sin(state.clock.elapsedTime * 0.08 + depth) *
        (performanceTier === "low" ? 0.08 : 0.16) +
        driftY * depthWeight * 0.12,
      delta * 1.4,
    );
    points.position.z = MathUtils.lerp(
      points.position.z,
      scrollOffset * drift * depthWeight * 1.6,
      delta,
    );
  });
...
function GravityCore() {
  ...
  useFrame((state, delta) => {
    ...
    group.scale.setScalar(MathUtils.lerp(group.scale.x, heroWeight * targetScale, delta * 2.5));
    group.rotation.x = MathUtils.lerp(group.rotation.x, swayX, delta * 1.8);
    group.rotation.y += delta * (reducedMotion ? 0.08 : 0.22);
    group.rotation.z = MathUtils.lerp(
      group.rotation.z,
      swayY * 0.25 + Math.sin(state.clock.elapsedTime * 0.42) * 0.08,
      delta * 1.6,
    );
    group.position.x = MathUtils.lerp(group.position.x, driftX * 0.34, delta);
    group.position.y = Math.sin(state.clock.elapsedTime * 0.75) * 0.15;
  });
